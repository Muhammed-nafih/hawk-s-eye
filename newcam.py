...
def check(i,types,path):
    if types == 'criminal':
        ...
        q1 = db.selectOne("select thief_user.*,thief_camera.id as cid,thief_camera.camera_number from thief_camera,thief_user where thief_camera.USER_id=thief_user.id and thief_camera.USER_id='"+str(camerauid)+"'")
    ...
    if types == 'familear':
        db.insert("insert into thief_familiar_log values ('',curdate(),curtime(),'"+str(i)+"')")
